...
function ScratchIt(){
  ...
  var onCanvasLoaded = function(){
    ...
    console.log("Overlay canvas and brush were loaded");
  ...
  var loadAndPaintPointsFromSvg = function() {
    ...
    pathsArray.forEach((path, index) => {
      if (index === 0) {
        ...
        isPointerDown = true;
        let point = getPointFromSVGPath(path);
      ...
      else if (index === pathsArray.length - 1) {
        ...
        //Nth path
        let point = getPointFromSVGPath(path);
        console.log(`painting path at ${point.x} and ${point.y}`);
        ...
        //TODO this for moving along a path
        //var pointerPosition = getPointFromEvent(event);
        //     addPoint({
        //       x: (offsetOrigin.x + (pointerPosition.x - pointerOrigin.x)) * scale,
        //       y: (offsetOrigin.y + (pointerPosition.y - pointerOrigin.y)) * scale
  ...
  var getPointFromSVGPath = function(path) {

    var root = document.getElementById('svgcontainer').firstChild;
